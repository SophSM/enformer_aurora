...
def create_step_function(model, optimizer, clip_norm=0.2):
    def train_step(batch, head):
        model.train()
        optimizer.zero_grad()

        # Forward pass
        batch['sequence_human'] = batch['sequence_human'].to(device)
        batch['target_human'] = batch['target_human'].to(device)

        outputs = model(batch['sequence_human'])[head]
        target = batch['target_human']
...
enformer_params = dict(channels= 1536, num_heads=8, num_transformer_layers=11, prediction_head="human")
